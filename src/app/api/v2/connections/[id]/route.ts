...
// ─── GET /api/v2/connections/[id] — fetch single connection ─────
...
export const DELETE = withAuth(async (req, session) => {
  ...
  // TODO: When Report model is updated to use connectionId (Task 9),
  // add a check here similar to the old route:
  //   const reportCount = await prisma.report.count({ where: { connectionId: id } });
  //   if (reportCount > 0) return 409;
  // For now the Connection model has no Report relation, so no FK check needed.
