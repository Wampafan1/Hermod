...
export const POST = withAuth(async (req, session) => {
  ...
  const running = await prisma.mssqlBackupRun.count({ where: { policyId: id, type: "FULL", status: "RUNNING" } });
  if (running > 0) return NextResponse.json({ error: "A SQL Server full backup is already running" }, { status: 409 });
  const boss = await ensureBossStarted();
  await boss.send("mssql-backup-full", { policyId: id, triggeredBy: "manual" }, { singletonKey: `mssql-full-${id}` });
