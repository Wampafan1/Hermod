...
export const POST = withAuth(async (req, session) => {
  ...
  if (!policy.logFrequency) return NextResponse.json({ error: "Transaction log backups are disabled for this policy" }, { status: 409 });
  const running = await prisma.mssqlBackupRun.count({ where: { policyId: id, type: "LOG", status: "RUNNING" } });
  if (running > 0) return NextResponse.json({ error: "A SQL Server transaction log backup is already running" }, { status: 409 });
  const boss = await ensureBossStarted();
  await boss.send("mssql-backup-log", { policyId: id, triggeredBy: "manual" }, { singletonKey: `mssql-log-${id}` });
