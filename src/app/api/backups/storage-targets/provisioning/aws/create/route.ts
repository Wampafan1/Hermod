...
import { createAwsS3BackupTarget } from "@/lib/backups/provisioning/aws-provisioner";
...
export const POST = withAuth(async (req, session) => {
  ...
  const parsed = awsProvisioningRequestSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const provisioned = await createAwsS3BackupTarget(parsed.data);
  ...
  const target = await prisma.backupStorageTarget.create({
    data: {
      name: parsed.data.name ?? `AWS S3 ${parsed.data.bucket}`,
